...
from pyrogram.errors import AccessTokenInvalid, PeerIdInvalid
...
async def clone_txt(client, message: Message):
    ...
    try:
        # Clone Bot Client အသစ်တည်ဆောက်ခြင်း
        ai = Client(
            name=bot_token,
            api_id=API_ID,
            api_hash=API_HASH,
            bot_token=bot_token,
            plugins=dict(root="maythusharmusic.plugins"), # Main Bot ၏ Plugin များကို ယူသုံးမည်
        )
...
"""
        ...
        await msg.edit_text("❌ Bot Token မှားယွင်းနေပါသည်။ ကျေးဇူးပြု၍ စစ်ဆေးပါ။")
...
async def delete_clone_bot(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("<b>အသုံးပြုပုံ :</b> /delclone [Bot Token သို့မဟုတ် @Username]")
    
    token_or_user = message.text.split(None, 1)[1]
    await msg.reply_text("ခေတ္တစောင့်ပါ...")

    # ဒီနေရာမှာ Database ကနေ ရှာပြီး ဖျက်တဲ့ Logic ထပ်ဖြည့်နိုင်ပါတယ်
    # လောလောဆယ် ရိုးရှင်းအောင် Token နဲ့ဖျက်ခိုင်းပါမယ်
    
    await delete_clone(token_or_user)
    await message.reply_text("✅ Clone Bot ကို ဖျက်သိမ်းလိုက်ပါပြီ။")


# Bot စ run ချိန်တွင် ရှိပြီးသား Clone များကို ပြန်လည်အသက်သွင်းခြင်း

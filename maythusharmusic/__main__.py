...
async def init():
    ...
    LOGGER("maythusharmusic.plugins").info(
        "Successfully Imported Modules for ALL Clients..."
    )
    ...
    await asyncio.gather(*[client.start() for client in app])
    ...
    # --- (Cache Pre-load) ---
    # (မှတ်ချက်: YouTube.load_cache() function ကို သင်၏ youtubedatabase.py မှာ ထည့်သွင်းထားဖို့ လိုပါမယ်)
    ...
    await asyncio.gather(*[client.stop() for client in app])
    await userbot.stop()


if __name__ == "__main__":
    # (Python 3.11 အတွက် asyncio.run() ကို သုံးပါ)
    try:
        asyncio.run(init())
